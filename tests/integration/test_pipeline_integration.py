...
class TestIngestionToPreprocessingPipeline:
    ...
    def test_dataset_detector_dataframe_type_detection(self, sample_trader_df: pd.DataFrame) -> None:
    ...
    def test_preprocessed_output_has_required_columns(self, sample_trader_df: pd.DataFrame) -> None:
        """Preprocessed trader DataFrame retains all critical columns."""
        required = {"symbol", "side", "size", "execution_price", "closed_pnl", "timestamp"}
        assert required.issubset(set(sample_trader_df.columns)), f"Missing columns: {required - set(sample_trader_df.columns)}"

    def test_no_negative_prices_after_preprocessing(self, sample_trader_df: pd.DataFrame) -> None:
        """No negative execution prices exist after preprocessing."""
        assert (sample_trader_df["execution_price"] > 0).all()

    def test_no_negative_sizes_after_preprocessing(self, sample_trader_df: pd.DataFrame) -> None:
...
class TestFeatureEngineeringPipeline:
    ...
    def test_trade_value_equals_size_times_price(self, sample_trader_df: pd.DataFrame) -> None:
    ...
    def test_is_profit_flag_matches_closed_pnl_sign(self, sample_trader_df: pd.DataFrame) -> None:
    ...
    def test_cumulative_pnl_is_monotone_cumsum(self, sample_trader_df: pd.DataFrame) -> None:
    ...
    def test_time_feature_columns_are_present(self, sample_trader_df: pd.DataFrame) -> None:
...
class TestMergePipeline:
    ...
    def test_merged_df_contains_fg_columns(self, sample_merged_df: pd.DataFrame) -> None:
    ...
    def test_fg_classification_is_valid_category(self, sample_merged_df: pd.DataFrame) -> None:
    ...
    def test_merged_df_no_nan_in_critical_columns(self, sample_merged_df: pd.DataFrame) -> None:
...
class TestAnalyticsPipeline:
    ...
    def test_win_rate_is_between_0_and_1(self, sample_merged_df: pd.DataFrame) -> None:
    ...
    def test_pnl_by_sentiment_regime_has_all_categories(self, sample_merged_df: pd.DataFrame) -> None:
    ...
    def test_symbol_pnl_ranking_is_sorted(self, sample_merged_df: pd.DataFrame) -> None:
        """Symbol PnL ranking is descending."""
        ranking = sample_merged_df.groupby("symbol")["closed_pnl"].sum().sort_values(ascending=False)
        assert list(ranking.values) == sorted(list(ranking.values), reverse=True)

    def test_total_pnl_matches_sum_of_closed_pnl(self, sample_merged_df: pd.DataFrame) -> None:
