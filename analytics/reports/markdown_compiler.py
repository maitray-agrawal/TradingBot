...
class MarkdownReportCompiler:
    ...
    def compile_executive_summary(cls, analytics_res: Dict[str, Any], statistics_res: Dict[str, Any]) -> str:
        ...
        md.append(
            f"| **Profit Factor** | {cls._format_float(perf.get('profit_factor', 0), 2)} | Ratio of gross profits to gross losses |"
        )
        md.append(
            f"| **Average Trade return** | {cls._format_currency(perf.get('average_pnl', 0))} | Mean profit/loss per trade |"
        )
        ...
        md.append(
            f"| **Composite Risk Score** | {cls._format_float(risk.get('composite_risk_score', 0), 2)} / 10 | Standardized portfolio risk score |"
        )
        md.append("")
        ...
        md.append(
            f"- **Top Performing Asset**: `{best_symbol}` with a total realized PnL of **{cls._format_currency(best_pnl)}**."
        )
        md.append(
            f"- **Bottom Performing Asset**: `{worst_symbol}` with a total realized PnL of **{cls._format_currency(worst_pnl)}**."
        )
        md.append("")
    ...
    def compile_technical_summary(cls, analytics_res: Dict[str, Any], statistics_res: Dict[str, Any]) -> str:
        ...
        md.append(
            f"| **Standard Deviation** | {cls._format_float(pnl_stats.get('std'))} | {cls._format_float(vol_stats.get('std'))} |"
        )
        md.append(
            f"| **Skewness** | {cls._format_float(pnl_stats.get('skew'))} | {cls._format_float(vol_stats.get('skew'))} |"
        )
        md.append(
            f"| **Kurtosis** | {cls._format_float(pnl_stats.get('kurtosis'))} | {cls._format_float(vol_stats.get('kurtosis'))} |"
        )
        md.append(
            f"| **Min Value** | {cls._format_currency(pnl_stats.get('min'))} | {cls._format_float(vol_stats.get('min'))} |"
        )
        md.append(
            f"| **Median (50%)** | {cls._format_currency(pnl_stats.get('50%'))} | {cls._format_float(vol_stats.get('50%'))} |"
        )
        md.append(
            f"| **Max Value** | {cls._format_currency(pnl_stats.get('max'))} | {cls._format_float(vol_stats.get('max'))} |"
        )
        md.append("")
        ...
        for param, details in ci.items():
            if isinstance(details, dict):
                ...
                if "win_rate" in param:
                    md.append(
                        f"| {name} | {cls._format_pct(est)} | {cls._format_pct(low)} | {cls._format_pct(up)} | Wilson Score |"
                    )
                else:
                    md.append(
                        f"| {name} | {cls._format_currency(est)} | {cls._format_currency(low)} | {cls._format_currency(up)} | {ci_type} |"
                    )
        md.append("")
        ...
        md.append(
            f"- **Cohen's d (Fear vs Greed Returns)**: {cls._format_float(cohen.get('value'))} ({cohen.get('interpretation', 'N/A')})"
        )
        md.append(
            f"- **Eta-squared (Sentiment Class Variance)**: {cls._format_float(eta.get('value'))} ({eta.get('interpretation', 'N/A')})"
        )
        md.append("")
    ...
    def compile_business_report(cls, analytics_res: Dict[str, Any], statistics_res: Dict[str, Any]) -> str:
        ...
        md.append("| :--- | :--- | :--- |")
        md.append(
            f"| **Gross Realized Profit** | {cls._format_currency(perf.get('gross_profit', 0))} | Sum of all profitable futures trades |"
        )
        md.append(
            f"| **Gross Realized Loss** | {cls._format_currency(perf.get('gross_loss', 0))} | Sum of all losing futures trades |"
        )
        md.append(
            f"| **Net Portfolio Return** | {cls._format_currency(perf.get('net_profit', 0))} | Combined net profitability |"
        )
        ...
        md.append(
            f"| **Total Brokerage Fees** | {cls._format_currency(perf.get('total_fees', 0))} | Realized Binance Futures trading fees |"
        )
        md.append("")
        ...
        md.append("| :--- | :--- | :--- |")
        md.append(
            f"| **Portfolio Volatility (Realized)** | {cls._format_float(risk.get('pnl_volatility', 0), 4)} | Trade-by-trade standard deviation |"
        )
        md.append(
            f"| **Max Realized Drawdown** | {cls._format_pct(risk.get('max_drawdown', 0))} | Peak-to-trough drop in realized PnL |"
        )
        md.append(
            f"| **Value at Risk (95% VaR)** | {cls._format_currency(risk.get('value_at_risk_95', 0))} | Potential loss at 95% confidence level |"
        )
        md.append(
            f"| **Expected Shortfall (95% ES)** | {cls._format_currency(risk.get('expected_shortfall_95', 0))} | Average loss in the worst 5% of cases |"
        )
        md.append(
            f"| **Profit-Loss Ratio** | {cls._format_float(risk.get('profit_loss_ratio', 0), 2)} | Average profit divided by average loss |"
        )
        md.append(
            f"| **Composite Risk Classification** | **{risk.get('risk_classification', 'Unknown')}** | Overall portfolio risk state |"
        )
        md.append("")
