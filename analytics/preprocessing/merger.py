...
class DatasetMerger:
    ...
    def merge_datasets(
        ...
        fg_df.rename(columns={
            "value": "fg_value",
            "classification": "fg_classification"
        }, inplace=True)

        analytics_logger.info(f"Merging trader data ({len(trader_df)} rows) with Fear & Greed data ({len(fg_df)} rows) using strategy: {strategy}...")
