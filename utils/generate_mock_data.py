...
def generate_mock_datasets():
    ...
    for i in range(150):
        ...
        size = round(np.random.uniform(0.01, 1.5) if symbol == "BTCUSDT" else np.random.uniform(0.1, 15.0), 4)
    ...
    trader_df = pd.concat([trader_df, trader_df.iloc[[5, 10]], pd.DataFrame([{
        "Account": "SubAccount_1",
        "Coin": "BTC",
        "Symbol": "BTCUSDT",
